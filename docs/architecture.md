...
| 路径 | 职责 |
|------|------|
| `server/` | Fastify 应用、路由、扫描器、版本与回收站存储、ClawHub 与市场聚合等集成；`app.ts` 导出 `buildApp()`（供进程入口与 Vitest `inject` 测试复用） |
| `server/routes/` | 按领域划分的 HTTP 路由（skills、manage、versions、similarity、trash、clawhub、marketplace） |
...
| `web/` | Vite 根目录；`web/src` 为 React 源码（组件、hooks）；技能市场为 **聚合搜索**（`MarketplaceAggregatedView` + `POST /api/marketplace/search`），仅 ClawHub 兼容 HTTP 源 |
| `skills/` | 仓库内文档型 SKILL（`skill-hub-search` 等），供 Cursor/Claude 阅读；**不是**用户本机安装路径。运行时由 `GET /api/skill-hub/docs*` 读取 `skills/<id>/SKILL.md`，Web UI 以折叠面板展示 |
...
- 前端为单页应用（SPA）。生产环境在已找到静态根目录时，对**非** `/api*`、`/ws` 的请求若未命中静态文件，会回退 `index.html`（见 `server/app.ts` 中 `setNotFoundHandler`），因此带查询参数的入口地址（如 `/?view=trash`）刷新后仍可加载应用。
- 顶栏当前视图与查询参数 **`view`** 同步（`history.pushState` / `popstate`），合法取值与行为见 [开发与调试](./development.md) 中的「前端 URL 与顶栏视图」一节。

## 延伸阅读

- 启动与网络行为的条目化说明：[本机运行契约（facts/studio.md）](./facts/studio.md)
- 诊断 JSON 示例：[示例输出与 JSON 形状](./facts/examples-and-output.md)
